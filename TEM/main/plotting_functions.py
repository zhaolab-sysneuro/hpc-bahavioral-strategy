...
def line_plot_smooth(cells, env, pars, plot_specs, name='sq', lims=(), mask=False, env_class=None, fig_dir=None, smooth=False):
    cell = cells[env]
    ...
    if plot_specs.split_freqs:
        ...
        n_cols = np.ceil(np.sqrt(n * 8 / x_dim))
        n_rows = np.ceil(np.sqrt(n * x_dim / 8))
    else:
    ...
    for grid in range(n):
        ...
        if smooth:
            ...
            yi = np.linspace(-0.4, 0.4, 100)
            xi, yi = np.meshgrid(xi, yi)
            # Interpolate the cell values on the grid
            from scipy.interpolate import griddata
            zi = griddata(xs, cell_prepared, xi, method='cubic')

            # Plot the heatmap
            contour = ax.contourf(xi, yi, zi, cmap=plot_specs.cmap, levels=20)
        else:
        ...
        ax.set_ylim(np.min(ys) - 0.4, np.max(ys) + 0.4)
...
def square_plot_smooth(cells, env, pars, plot_specs, name='sq', lims=(), mask=False, env_class=None, fig_dir=None, smooth=False):
    cell = cells[env]
    ...
    for grid in range(n):
        ...
        if smooth:
            ...
            # Interpolate the cell values on the grid
            from scipy.interpolate import griddata
            zi = griddata((xs, ys), cell_prepared, (xi, yi), method='cubic')
...
def get_data(save_dirs, run, date, recent=-1, index=None, smoothing=0, n_envs_save=None):
    ...
    g_gen_timeseries = load_numpy_gz(save_path + '/g_gen_timeseries_' + index + '.npy')
    p_gen_timeseries = load_numpy_gz(save_path + '/p_gen_timeseries_' + index + '.npy')
    p_x_timeseries = load_numpy_gz(save_path + '/p_x_timeseries_' + index + '.npy')
    ...
    p_x_all = rate_map_from_timeseries(p_x_timeseries, pos_timeseries, params, smoothing=smoothing, envs=envs)
    g_gen_all = rate_map_from_timeseries(g_gen_timeseries, pos_timeseries, params, smoothing=smoothing, envs=envs)
    p_gen_all = rate_map_from_timeseries(p_gen_timeseries, pos_timeseries, params, smoothing=smoothing, envs=envs)
    ...
    p_x_all = np.nan_to_num(p_x_all)
    g_gen_all = np.nan_to_num(g_gen_all)
    p_gen_all = np.nan_to_num(p_gen_all)
    ...
    data = DotDict({
        ...
        'p_x_timeseries': p_x_timeseries,
        'p_gen_timeseries': p_gen_timeseries,
        'g_gen_timeseries': g_gen_timeseries,
