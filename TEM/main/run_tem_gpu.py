...
# export LD_LIBRARY_PATH=$(find $CONDA_PREFIX/lib/python*/site-packages/nvidia -type d -name lib | paste -sd ":" -):$LD_LIBRARY_PATH
...
for file in files:
    if os.path.isfile(file):
        subprocess.run(['cp', file, os.path.join(script_path, file)], check=True)

# Initialise hyper-parameters for model
params = parameters.default_params(world_type='zhao2025', batch_size = 32, width = 5, height = None)
params.train_iters = 100000
# Save parameters
...
for train_i in range(params.train_iters):
    ...
    if sum(train_dict.env_steps == 0) > 0:
        ...
        logger_envs.info(msg)
    if (train_i) % 10 == 0:
        print(f'training iter {train_i}/{params.train_iters}')
