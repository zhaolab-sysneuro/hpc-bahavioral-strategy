...
class TEM(tf.keras.Model):
    ...
    def g2g(self, g, t_mat, name=''):
        ...
        :param name: whether generative or inference
        :return:
        """
    ...
    def f_g_freq(self, g, _):
        return g #tf.minimum(tf.maximum(g, -1), 1)

    @model_utils.define_scope
    def f_p_freq(self, p, _):
        return p #tf.minimum(tf.maximum(p, -1), 1)
...
def compute_losses(model_inputs, data, trainable_variables, par):
    ...
    for i in range(par.seq_len):
        ...
        if par.world_type in ['zhao2025'] and par.use_reward:
        ...
        lg_reg_ = tf.reduce_sum(tf.abs(data.g.g[i]), axis=1)
        ...
        lp += tf.reduce_sum(lp_ * s_vis) * scalings.temp * norm * par.scal_p
        lg += tf.reduce_sum(lg_ * s_vis) * scalings.temp * norm * par.scal_g
        lp_x += tf.reduce_sum(lp_x_ * s_vis) * scalings.p2g_use * scalings.temp * norm * par.scal_p

        lg_reg += tf.reduce_sum(lg_reg_ * s_vis) * par.g_reg_pen * scalings.g_cell_reg * norm * par.scal_g
        lp_reg += tf.reduce_sum(lp_reg_ * s_vis) * par.p_reg_pen * scalings.p_cell_reg * norm * par.scal_p
