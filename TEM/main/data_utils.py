...
def prepare_cell_timeseries(data, prev_data, pars):
    gs, ps, g_gen, p_gen, p_x, pos, xs, xs_gt = data
    gs_, ps_, g_gen_, p_gen_, p_x_, pos_, xs_, xs_gt_ = prev_data
    # convert to batch_size x cells x timesteps
    g1s = np.transpose(np.array(cp.deepcopy(gs)), [1, 2, 0])
    p1s = np.transpose(np.array(cp.deepcopy(ps)), [1, 2, 0])
    g1s_gen = np.transpose(np.array(cp.deepcopy(g_gen)), [1, 2, 0])
    p1s_gen = np.transpose(np.array(cp.deepcopy(p_gen)), [1, 2, 0])
    p1s_x = np.transpose(np.array(cp.deepcopy(p_x)), [1, 2, 0])
    g1s = g1s[:pars.n_envs_save, :, :]
    p1s = p1s[:pars.n_envs_save, :, :]
    g1s_gen = g1s_gen[:pars.n_envs_save, :, :]
    p1s_gen = p1s_gen[:pars.n_envs_save, :, :]

    xgt1s = np.transpose(np.array(cp.deepcopy(xs_gt)), [1, 2, 0])
    xgt1s = xgt1s[:pars.n_envs_save, :, :]
    x1s = xs[:pars.n_envs_save, :, :]

    grids, places, grids_gen, places_gen, places_x, positions, senses, senses_pred = [], [], [], [], [], [], [], []

    for env in range(pars.n_envs_save):
        if gs_ is None:
            grids.append(cp.deepcopy(g1s[env]))
            places.append(cp.deepcopy(p1s[env]))
            grids_gen.append(cp.deepcopy(g1s_gen[env]))
            places_gen.append(cp.deepcopy(p1s_gen[env]))
            places_x.append(cp.deepcopy(p1s_x[env]))
            ...
            grids_gen.append(np.concatenate((g_gen_[env], g1s_gen[env]), axis=1))
            places_gen.append(np.concatenate((p_gen_[env], p1s_gen[env]), axis=1))
            places_x.append(np.concatenate((p_x_[env], p1s_x[env]), axis=1))
    ...
    return [grids, places, grids_gen, places_gen, places_x, positions, senses, senses_pred]
...
def initialise_environments(curric_env, env_steps, pars, test=False):
    for b, (env, env_step) in enumerate(zip(curric_env.envs, env_steps)):
        ...
        if pars.world_type == 'zhao2025':
            curric_env.envs[b] = environments.Zhao2025(pars, pars.env.widths[b], test=test)
...
def save_model_outputs(model, model_utils_, train_i, save_path, pars):
    ...
    gs_timeseries, ps_timeseries, g_gen_timeseries, p_gen_timeseries, p_x_timeseries, pos_timeseries, xs_timeseries, xs_gt_timeseries, variables_test = \
    ...
    while data_continue:
        ...
        g_gen_numpy = [x.numpy() for x in variables_test.g.g_gen]
        p_gen_numpy = [x.numpy() for x in variables_test.p.p_g]
        p_x_numpy = [x.numpy() for x in variables_test.p.p_x]
        ps_numpy = [x.numpy() for x in variables_test.p.p]
        x_gt_numpy = [x.numpy() for x in variables_test.pred.x_gt]

        # Update timeseries
        prev_cell_timeseries = [gs_timeseries, ps_timeseries, g_gen_timeseries, p_gen_timeseries, p_x_timeseries, pos_timeseries, xs_timeseries, xs_gt_timeseries]
        save_data_timeseries = [gs_numpy, ps_numpy, g_gen_numpy, p_gen_numpy, p_x_numpy, position, xs, x_gt_numpy]
        cell_timeseries = prepare_cell_timeseries(save_data_timeseries, prev_cell_timeseries, pars)
        gs_timeseries, ps_timeseries, g_gen_timeseries, p_gen_timeseries, p_x_timeseries, pos_timeseries, xs_timeseries, xs_gt_timeseries = cell_timeseries
    ...
    np.save(save_path + '/g_gen_timeseries_' + str(train_i), g_gen_timeseries)
    np.save(save_path + '/p_gen_timeseries_' + str(train_i), p_gen_timeseries)
    np.save(save_path + '/p_x_timeseries_' + str(train_i), p_x_timeseries)
