...
class Zhao2025(Environment):
    ...
    def __init__(self, params, width):
        ...
        self.reward_pos_training = self.reward_A_pos + self.reward_B_pos + self.reward_C_pos
        self.reward_value = self.n_states
    ...
    def state_data(self):
        ...
        if self.par.use_reward:
            # choose reward sense
            reward_A_sense = np.random.choice(choices)
            reward_B_sense = np.random.choice(choices)
            reward_C_sense = np.random.choice(choices)
            start_sense = np.random.choice(choices)
            end_sense = np.random.choice(choices)
            # choices = np.delete(choices, reward_sense)
        else:
            reward_sense, no_reward_sense = 0, 0
        ...
        for i in range(self.n_states):
            new_state = np.random.choice(choices)
            width = self.width
            states_vec[i] = new_state if i / width < 1 else states_vec[i - width]
